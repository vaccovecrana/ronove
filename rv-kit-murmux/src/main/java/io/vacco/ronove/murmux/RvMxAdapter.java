...
public class RvMxAdapter<Api> extends RvAdapter<Api, MxHandler, MxExchange> {
  ...
  @Override
  public String loadPath(RvParameter pp, MxExchange x) {
    return x.getPathParam(pp.name);
  }

  @Override
  public String loadQuery(RvParameter qp, MxExchange x) {
    return x.getQueryParam(qp.name);
  }

  @Override
  public String loadCookie(RvParameter cp, MxExchange x) {
    var c = x.cookies.get(cp.name);
    return c != null ? c.value : null;
  }

  @Override
  public String loadForm(RvParameter fp, MxExchange x) {
    return x.getFormParam(fp.name);
  }

  @Override
  public String loadHeader(RvParameter hp, MxExchange x) {
    return x.io.getRequestHeaders().getFirst(hp.name);
  }

  @Override
  public Object loadAttachment(RvParameter ap, RvAttachmentParam at, MxExchange x) {
  ...
  @Override
  public MxHandler combine(List<RvHandler<MxExchange>> rvHandlers) {
  ...
  @Override
  public void commitResponse(RvResponse<?> res, MxExchange x) {
